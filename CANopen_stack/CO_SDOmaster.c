...
INTEGER16 CO_SDOclient_receive(void *object, CO_CANrxMsg_t *msg){
   ...
   SDO_C->CANrxData[0] = msg->_data[0];
   SDO_C->CANrxData[1] = msg->_data[1];
   SDO_C->CANrxData[2] = msg->_data[2];
   SDO_C->CANrxData[3] = msg->_data[3];
   SDO_C->CANrxData[4] = msg->_data[4];
   SDO_C->CANrxData[5] = msg->_data[5];
   SDO_C->CANrxData[6] = msg->_data[6];
   SDO_C->CANrxData[7] = msg->_data[7];
...
/******************************************************************************/
void CO_SDOclient_abort(CO_SDOclient_t *SDO_C, UNSIGNED32 thecode){
   SDO_C->CANtxBuff->_data[0] = 0x80;
   SDO_C->CANtxBuff->_data[1] = SDO_C->index & 0xFF;
   SDO_C->CANtxBuff->_data[2] = (SDO_C->index>>8) & 0xFF;
   SDO_C->CANtxBuff->_data[3] = SDO_C->subIndex;
   memcpySwap4(&SDO_C->CANtxBuff->_data[4], (UNSIGNED8*)&thecode);
                                       ...
                                       UNSIGNED16        dataSize)
{
   ...
   SDO_C->CANtxBuff->_data[1] = index & 0xFF;
   SDO_C->CANtxBuff->_data[2] = index >> 8;
   SDO_C->CANtxBuff->_data[3] = subIndex;
   ...
   if(dataSize <= 4){
      UNSIGNED16 i;
      //expedited transfer
      SDO_C->CANtxBuff->_data[0] = 0x23 | ((4-dataSize) << 2);
      //clear unused data bytes
      SDO_C->CANtxBuff->_data[4] = 0;
      SDO_C->CANtxBuff->_data[5] = 0;
      SDO_C->CANtxBuff->_data[6] = 0;
      SDO_C->CANtxBuff->_data[7] = 0;
      //copy data
      for(i=dataSize+3; i>=4; i--) SDO_C->CANtxBuff->_data[i] = dataTx[i-4];
   }
   else{
      UNSIGNED32 len;
      //segmented transfer
      SDO_C->CANtxBuff->_data[0] = 0x21;
      len = dataSize;
      memcpySwap4(&SDO_C->CANtxBuff->_data[4], (UNSIGNED8*)&len);
   }
   ...
   *pSDOabortCode = 0;
   ...
   if(SDO_C->CANrxNew){
      ...
      if(SDO_C->state & 0x02){
         ...
         //fill data bytes
         for(i=0; i<j; i++)
            SDO_C->CANtxBuff->_data[i+1] = SDO_C->buffer[SDO_C->bufferOffset + i];
         for(; i<7; i++)
            SDO_C->CANtxBuff->_data[i+1] = 0;
         SDO_C->bufferOffset += j;
         //SDO command specifier
         SDO_C->CANtxBuff->_data[0] = 0x00 | (SDO_C->state&0x10) | ((7-j)<<1);
         //is end of transfer?
         if(SDO_C->bufferOffset == SDO_C->bufferSize){
            SDO_C->CANtxBuff->_data[0] |= 1;
         }
   ...
   *pSDOabortCode = 0;
   ...
   if(SDO_C->CANrxNew){
      ...
      if(SDO_C->state & 0x04){
         ...
         SDO_C->CANtxBuff->_data[0] = 0x60 | (SDO_C->state&0x10);
         SDO_C->CANtxBuff->_data[1] = 0;
         SDO_C->CANtxBuff->_data[2] = 0;
         SDO_C->CANtxBuff->_data[3] = 0;
         SDO_C->CANtxBuff->_data[4] = 0;
         SDO_C->CANtxBuff->_data[5] = 0;
         SDO_C->CANtxBuff->_data[6] = 0;
         SDO_C->CANtxBuff->_data[7] = 0;
