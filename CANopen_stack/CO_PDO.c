...
INTEGER16 CO_PDO_receive(void *object, CO_CANrxMsg_t *msg){
   ...
   if(RPDO->valid && *RPDO->operatingState == CO_NMT_OPERATIONAL){
      ...
      RPDO->CANrxData[0] = msg->_data[0];
      RPDO->CANrxData[1] = msg->_data[1];
      RPDO->CANrxData[2] = msg->_data[2];
      RPDO->CANrxData[3] = msg->_data[3];
      RPDO->CANrxData[4] = msg->_data[4];
      RPDO->CANrxData[5] = msg->_data[5];
      RPDO->CANrxData[6] = msg->_data[6];
      RPDO->CANrxData[7] = msg->_data[7];
...
UNSIGNED8 CO_TPDOisCOS(CO_TPDO_t *TPDO){
   ...
   pPDOdataByte = &TPDO->CANtxBuff->_data[TPDO->dataLength];
...
INTEGER16 CO_TPDOsend(CO_TPDO_t *TPDO){
   ...
   pPDOdataByte = &TPDO->CANtxBuff->_data[0];
...
unsigned long ul_num_of_rpdo_rx;
unsigned long ul_clock_at_last_rpdo;
/******************************************************************************/
void CO_RPDO_process(CO_RPDO_t *RPDO){
    extern volatile unsigned long ul_counter_interrupt_timer0;
    if(RPDO->CANrxNew && RPDO->valid && *RPDO->operatingState == CO_NMT_OPERATIONAL){
        ...
        ul_num_of_rpdo_rx++;
        ul_clock_at_last_rpdo=ul_counter_interrupt_timer0;
