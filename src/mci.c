...
#if !MCI_DMA_ENABLED
    volatile BYTE WriteBlock[1024];
    volatile BYTE ReadBlock[1024];
#else
...
Tipo_Struct_SD SD __attribute__ ((aligned(32)));

typedef struct _TipoCardInfo{
	unsigned int uiPhysSpec2supported;
	unsigned int uiHiCapacity;
}TipoCardInfo;
TipoCardInfo cardInfo;
...
// Funzione che data una periferica permette di conoscere il valore di pclk della periferica stessa
// (es timer0, timer1, i2cbus)
unsigned long ulGetPclk(enumLPC_Peripheral p){
	unsigned int uiPCLKSEL_value,uiPCLKSEL_mask,uiPCLKSEL_shift;
	unsigned int ui_PclkDivider;
	unsigned long ul_Pclk;
	if (p>=enumLPC_Peripheral_numOf)
		return Fcclk;
	if (p>=enumLPC_Peripheral_PCLK_BAT_RAM){
		uiPCLKSEL_value=PCLKSEL1;
		uiPCLKSEL_shift=2*(p-enumLPC_Peripheral_PCLK_BAT_RAM);
		uiPCLKSEL_mask=((3)<<uiPCLKSEL_shift);
	}
	else{
		uiPCLKSEL_value=PCLKSEL0;
		uiPCLKSEL_shift=2*(p-enumLPC_Peripheral_PCLK_WDT);
		uiPCLKSEL_mask=((3)<<uiPCLKSEL_shift);
	}
	switch((uiPCLKSEL_value&uiPCLKSEL_mask)>>uiPCLKSEL_shift){
		case 0:
			ui_PclkDivider=4;
			break;
		case 1:
		...
		case 3:
		default:
			if (  (p==enumLPC_Peripheral_PCLK_CAN1)
				||(p==enumLPC_Peripheral_PCLK_CAN2)
				)
				ui_PclkDivider=6;
			else
				ui_PclkDivider=8;
...
void MCI_Set_MCIClock( DWORD ClockRate )
{
  //DWORD i;
  int pClk,clkDiv;
// cfr lpc2388 user manual, page 495

// get mci peripheral clock
  pClk=ulGetPclk(enumLPC_Peripheral_PCLK_MCI);

// using slow rate?  
  if ( ClockRate == SLOW_RATE ) {
//    clkDiv=pClk/(2*MCLKDIV_SLOW_HZ)-1;   
// using these settings we can obtain a 394kHz frequency, else with the -1 we get 405kHz, too high  
    clkDiv=pClk/(2*MCLKDIV_SLOW_HZ);   
  }
  else {
//    clkDiv=pClk/(2*MCLKDIV_NORMAL_HZ)-1;
    clkDiv=pClk/(2*MCLKDIV_NORMAL_HZ);
  }
  // saturate...
  if (clkDiv>0xFF)
      clkDiv=0xFF;
  ...
  MCI_CLOCK |= (1 << 8)  |clkDiv; // set new clock divider and enable the mci clock
...
DWORD MCI_Init( void )
{
  ...
  /*connect MCI signals to P0.19-P0.22, and P2.11-P2.13*/
  {
	unsigned long ul;
	ul=PINSEL1;
	// reset settings on P0.19...P0.22
	ul &=~( (3<<12)|(3<<10)|(3<<8)|(3<<6));
	// set settings on P0.19...P0.22
	ul |= ( (2<<12)|(2<<10)|(2<<8)|(2<<6));
	PINSEL1 = ul; // that is about 0x2A80;
	
	ul=PINSEL4;
	// reset settings on P2.11...P2.13
	ul &=~( (3<<22)|(3<<24)|(3<<26));
	// set settings on P2.11...P2.13
	ul |= ( (2<<22)|(2<<24)|(2<<26));
	PINSEL4 = ul; // that is about 0x0A80 0000;
  }
  
// select pinmode: all the pins with pullup resistor
// PULLUP ON CLK, CMD, DATA0, PWR  
  PINMODE1=PINMODE1&~(   (3<<((19-16)*2))
                        |(3<<((20-16)*2))
                        |(3<<((21-16)*2))
                        |(3<<((22-16)*2))
                    );
// PULLUP ON DATA1, DATA2, DATA3
  PINMODE4=PINMODE4&~(   (3<<((11- 0)*2))
                        |(3<<((12- 0)*2))
                        |(3<<((13- 0)*2))
                    );
  ...
  // useless, our sd card is always powered on -->v_wait_microseconds(1200);
  //for ( i = 0; i < 0x100; i++ );
...
// wait for clock generation: at least 100 clock pulses should be generated during this phase
  // 100 clock /400 *1000=1/4000s=250us
  v_wait_microseconds(1200);
// horridus delay  
  //for ( i = 0; i < 0x2000; i++ );
      
    VICIntSelect &= ~(1<<MCI_INT); // MCI is a IRQ
    VICVectPriority24 = HIGHEST_PRIORITY ; // set priority for IRQ to be quite high
    VICVectAddr24 = (unsigned long)MCI_IRQHandler;
    VICIntEnable |= 1<<MCI_INT; // MCI interrupt enabled
...
//  for ( i = 0; i < 0x100; i++ );	
  ...
  MCI_COMMAND = CmdData;
// if no reply required, wait some time so some clocks can output and reset the sd card else maybe I can send 
// another command be4 the sd card is ready to reply...
// this was happening between cmd0 and cm8, cmd0 won't reply, so lpc continues with cm8 but sd card isn't ready to reply and hangs!
  if ( ExpectResp == EXPECT_NO_RESP )
// little delay waiting for some clock outputs to the sd card so it can reset
    v_wait_microseconds(500);
  else
// @10MHz, 8 clock lasts about 1us      
...
DWORD MCI_Send_ACMD_OP_Cond( void )
{
  ...
  while ( retryCount > 0 )
  {
	...
	// se card di tipo hc, comunico che supporto hc mode (bit 30) e che voglio max performances (bit 28)
	// non imposto bit24 perch� non voglio fare switch a 1.8V
	// OCR_INDEX imposta ad 1 tutti i bit del voltage range da 2.7 a 3.6 V
	// in mod da ammettere tutte le tensioni di alimentazione
	if (cardInfo.uiPhysSpec2supported){
		CmdArgument = OCR_INDEX|(1<<30)|(1<<28);
	}
	else{
		CmdArgument = OCR_INDEX;
	}
	
	MCI_SendCmd( SEND_APP_OP_COND, CmdArgument, EXPECT_SHORT_RESP, 0 );
	respStatus = MCI_GetCmdResp( SEND_APP_OP_COND, EXPECT_SHORT_RESP, (DWORD *)&respValue[0] );
	if ( !(respStatus & MCI_CMD_TIMEOUT) && (respValue[0] & 0x80000000) )
	{
		// check if the card is an hi capacity one
		if (respValue[0]&(1<<30)){
			cardInfo.uiHiCapacity=1;
		}
		return ( TRUE );	/* response is back and correct. */
	}
	//for ( i = 0; i < 0x20; i++ );
...
// issue cmd8: if returns ok, the card supports spec2.0, so it can be a hi capacity card  
unsigned int ui_send_cmd8(void){
	DWORD argument;
	DWORD respValue,respStatus;
	unsigned int ui_retcode;
	// initialize to non ok retcode
	ui_retcode=0;
	// argument 1AA means:
	// 0x01 --> voltage supplied is 3.3V
	// 0xaa --> the check pattern is 0xaa
	
	argument=0x000001AA;
	MCI_SendCmd( 8, argument, EXPECT_SHORT_RESP, 1 );
	// the expected reply is same as the argument, please check the voltage admitted and the  pattern
	respStatus = MCI_GetCmdResp( 8, EXPECT_SHORT_RESP, (DWORD *)&respValue );
	if ( respStatus == 0 )
	{
	  if (respValue==0x000001AA){
		// initialize to  ok retcode
		ui_retcode=1;
	  }
	}
	return ui_retcode;
...
// secondo specifiche sd card, dobbiamo spedire adesso cmd8  
	if(ui_send_cmd8()){
		cardInfo.uiPhysSpec2supported=1;
	}
	else{
		cardInfo.uiPhysSpec2supported=0;
	}
...
//Michele: non verifichiamo MMC card, non ci interessa!!
	#if 0
...
// salva il regsitro CSD nella struttura SD
void vSaveCSD_reg(DWORD *pSrc){
	DWORD *pDst;
	unsigned int i;
	pDst=(DWORD*)&(SD.csd);
	//pSRC[0] contiene la parte ALTA della risposta...
	//pSRC[3] contiene la parte bassa della risposta...
    i=0;
	while(i<sizeof(SD.csd)/sizeof(DWORD)){
		*pDst++=pSrc[sizeof(SD.csd)/sizeof(DWORD)-i-1];
        i++;
	}
}//void vSaveCSD_reg(DWORD *p)

// procedura di calcolo della capacit� dell'sd card
// deve essere messo a disposizione il registro CSD
void vCalcSD_capacity(void){
	unsigned long ulMULT,ulC_SIZE,ulC_SIZE_MULT;
	unsigned long ulC_SIZE_1,ulC_SIZE_2,ulC_SIZE_3;
	unsigned int i;
// csd_structure=0--> version 1
// csd_structure=1--> version 2
// else csd_structure unknown!
	if (SD.csd.CSD_STRUCTURE==1){
		Tipo_Struct_CSD_V2 *pcsd2=(Tipo_Struct_CSD_V2*)&(SD.csd);
		ulC_SIZE_1=pcsd2->C_SIZE_MULT_1;		// xxxb R [55:48]
		ulC_SIZE_2=pcsd2->C_SIZE_MULT_2;		// xxxb R [63:56]
		ulC_SIZE_3=pcsd2->C_SIZE_MULT_3;		// xxxb R [69:64]
		ulC_SIZE=ulC_SIZE_1+(ulC_SIZE_2*256)+(ulC_SIZE_3*65536);
		SD.ulSizeOfBlocks=512;
		SD.ulMemoryCapacity_Bytes=(ulC_SIZE+1)*512*1024;
		SD.ulNumberOfBlocks=(ulC_SIZE+1)*1024;
		SD.ulMemoryCapacity_KBytes=SD.ulMemoryCapacity_Bytes/1024;
		SD.ulMemoryCapacity_MBytes=SD.ulMemoryCapacity_Bytes/(1024*1024);
	}
	else{
		...
		// per questioni di allineamento, il blocco c_size � stato diviso
		// in un blocco da due bit + un blocco da 10bit
		ulC_SIZE_1=SD.csd.C_SIZE_1;

		ulC_SIZE_2=SD.csd.C_SIZE_2;
		ulC_SIZE_2*=4;

		ulC_SIZE_3=SD.csd.C_SIZE_3;
		ulC_SIZE_3*=1024;

		ulC_SIZE=ulC_SIZE_1+ulC_SIZE_2+ulC_SIZE_3;

		ulC_SIZE_MULT=(SD.csd.C_SIZE_MULT_1+2*SD.csd.C_SIZE_MULT_2)+2;
		ulMULT=1;
		i=0;
		while(i<ulC_SIZE_MULT){
			ulMULT*=2;
			i++;
		}
		SD.ulNumberOfBlocks=(ulC_SIZE+1)*ulMULT;
		SD.ulSizeOfBlocks=1;
		i=0;
		while(i<SD.csd.READ_BL_LEN){
			SD.ulSizeOfBlocks*=2;
			i++;
		}
		SD.ulMemoryCapacity_Bytes=SD.ulNumberOfBlocks*SD.ulSizeOfBlocks;
		SD.ulMemoryCapacity_KBytes=SD.ulMemoryCapacity_Bytes/1024;
		SD.ulMemoryCapacity_MBytes=SD.ulMemoryCapacity_Bytes/(1024*1024);
	}
}//void vCalcSD_capacity(void)
 ...
 ******************************************************************************/
// mthomas - static
DWORD MCI_Send_Write_Multiple_Block(DWORD blockNum) {
	DWORD retryCount;
	DWORD respStatus;
	DWORD respValue[4];
