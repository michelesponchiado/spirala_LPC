...
// ENABLE DMA INTERRUPTS...    
    VICIntSelect        &= ~(1<<GPDMA_INT);    // DMA selected as IRQ
    VICVectPriority25   = 1;            // set priority for DMA to be quite high
    VICVectAddr25       = (unsigned long)DMAHandler;
    VICIntEnable        |= (1<<GPDMA_INT);     // DMA interrupt enabled
