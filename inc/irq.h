...
//#define IENABLE asm ( "MRS sysreg, SPSR; MSR CPSR_c, #SYS32Mode");
#define IENABLE asm ( "MRS %[auxword], SPSR " :: [auxword] "Q" (sysreg)); asm("MSR CPSR_c, #SYS32Mode");
//asm ("fsinx %[angle],%[output]"
//: [output] "=f" (result)
//: [angle] "f" (angle));

//#define IDISABLE __asm { MSR CPSR_c, #(IRQ32Mode|I_Bit); MSR SPSR_cxsf, sysreg }
#define IDISABLE asm (" MSR CPSR_c, #(IRQ32Mode|I_Bit); MSR SPSR_cxsf, sysreg");

// usare queste define per riabilitare/disabilitare le interrupt nested
// vedi AN10381.pdf
// utilissimo per riabilitare interrupt i2c bus durante esecuzione interrupt timer 0
//#error riabilitare interrupt i2c bus durante timer0 interrupt
#define defIENABLE_nestedInts 	{asm ( "MRS LR, SPSR" );asm ( "STMFD SP!, {LR}" );asm ( "MSR CPSR_c, #0x1F" ) ;	asm ( "STMFD SP!, {LR}" );}

#define defIDISABLE_nestedInts  {asm ( "LDMFD SP!, {LR}" );	asm ( "MSR CPSR_c, #0x92");asm ( "LDMFD SP!, {LR}" );	asm ( "MSR SPSR_cxsf, LR" );}
