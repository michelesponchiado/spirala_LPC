...
#define ff_wtoupper(c) (toupper(c))
#define ff_convert(c,d) c
...
)
{
	...
	if (fmt == FS_FAT32) 
        ...
        i =N_ROOTDIR;	/* Number of rootdir entries */
	...
	for (i = 0; i < N_FATS; i++) {		/* Initialize each FAT copy */
		...
		if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
			return FR_DISK_ERR+0x34;
		mem_set(tbl, 0, SS(fs));			/* Fill following FAT entries with zero */
		for (n = 1; n < n_fat; n++) {		/* This loop may take a time on FAT32 volume due to many single sector writes */
			if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
				return FR_DISK_ERR+0x35;
		}
	}
    ...
    if (disk_ioctl(pdrv, CTRL_SYNC, 0) == RES_OK) 
        ...
        return FR_DISK_ERR+0x37;
	//return (disk_ioctl(pdrv, CTRL_SYNC, 0) == RES_OK) ? FR_OK : FR_DISK_ERR+0x37;
}
